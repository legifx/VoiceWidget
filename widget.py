#!/usr/bin/env python3
"""VoiceWidget — Dynamic Island Style. Weiße Insel, Lila/Blau Buttons."""
...
class Island(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.sr=16000; self._t0=0; self._ok=True
    ...
    def _idle(self):
        ...
        ctk.CTkLabel(r,"Voice",font=("Segoe UI",13,"bold"),text_color=DARK).pack(side="left")
        ctk.CTkLabel(r,"●",font=("Segoe UI",8),text_color=GN).pack(side="right",padx=6)
    ...
    def _rec_ui(self):
        ...
        self._rt=ctk.CTkLabel(r,"0:00",font=("Segoe UI",14,"bold"),text_color=DARK,width=40)
        self._rt.pack(side="left")
        self._vu=ctk.CTkLabel(r,"▁▁▁▁▁▁▁▁",font=("Segoe UI",10),text_color=PU,width=80)
        self._vu.pack(side="left",padx=(6,0))
        ctk.CTkLabel(r,"●",font=("Segoe UI",8),text_color=RD).pack(side="right",padx=8)

    def _load_ui(self):
        self._clr(); self.geometry(f"160x{self.H}"); self._isl.configure(border_color=GL)
        ctk.CTkLabel(self._isl,"⏳",font=("Segoe UI",16),text_color=GRAY).pack(expand=True)
    ...
    def _res_ui(self):
        ...
        self._sl=ctk.CTkLabel(top,"✅",font=("Segoe UI",10),text_color=GN)
        ...
        self._pv=ctk.CTkLabel(self._isl,"",font=("Segoe UI",12),text_color=FG,anchor="w",justify="left",wraplength=310)
        ...
        ctk.CTkOptionMenu(acts,variable=self._tv,values=["(keine)"],width=90,height=34,corner_radius=17,fg_color=GL,button_color=BL,button_hover_color=BLL,dropdown_fg_color=WHITE,dropdown_hover_color=BLL,text_color=DARK,font=("Segoe UI",11,"bold")).pack(side="left",padx=2)
