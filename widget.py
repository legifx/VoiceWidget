#!/usr/bin/env python3
"""VoiceWidget — Dynamic Island, Apple-Style. Weiss, Rund, minimal."""
import sys,os,json,configparser,threading,subprocess,io,time
from pathlib import Path

CFG=Path(__file__).parent/"config.ini"
D={"Server":{"host":"192.168.1.182","user":"server","port":"22","whisper_port":"8766"},
   "Widget":{"opacity":"1.0"}}
c=configparser.ConfigParser()
if CFG.exists():c.read(CFG)
for s,k in D.items():
    for kk,v in k.items():
        if not c.has_option(s,kk):c.set(s,kk,v)
with open(CFG,"w")as f:c.write(f)
H=c.get("Server","host");U=c.get("Server","user");WP=c.get("Server","whisper_port")
WU=f"http://{H}:{WP}";OP=float(c.get("Widget","opacity"))

import customtkinter as ctk
try:
    import sounddevice as sd,numpy as np,soundfile as sf
    HAS_AUDIO=True
except:
    sd=np=sf=None
    HAS_AUDIO=False

# ── Farben ──
W="#ffffff";BG="#f5f6fa";DARK="#1a1a2e";GR="#e0e3ea"
PU="#8b5cf6";PUL="#a78bfa";BL="#60a5fa";BLL="#93c5fd"
GN="#34d399";RD="#f87171";GY="#9ca3af";FG="#374151"
R=999  # volle Rundung

class Srv:
    def check(self):
        import urllib.request
        try:
            with urllib.request.urlopen(f"{WU}/health",timeout=4)as r:return json.loads(r.read())
        except:return{"error":"x"}
    def x(self,wav):
        import urllib.request
        bound=b"----Boundary123"
        b=b"--"+bound+b"\r\nContent-Disposition: form-data; name=\"file\"; filename=\"v.wav\"\r\nContent-Type: audio/wav\r\n\r\n"+wav+b"\r\n--"+bound+b"--\r\n"
        r=urllib.request.Request(f"{WU}/transcribe",data=b,headers={"Content-Type":f"multipart/form-data; boundary={bound.decode()}"})
        try:
            with urllib.request.urlopen(r,timeout=120)as f:return json.loads(f.read())
        except Exception as ex:return{"error":str(ex)[:30]}
    def tmux(self):
        try:
            r=subprocess.run(["ssh","-o","ConnectTimeout=3",f"{U}@{H}","tmux list-sessions -F '#{session_name}' 2>/dev/null"],capture_output=True,text=True,timeout=8)
            return[s.strip()for s in r.stdout.split("\n")if s.strip()]
        except:return[]
    def st(self,s,t):
        try:
            safe=t.replace("'","'\"'\"'").replace("\n","\\n")
            subprocess.run(["ssh","-o","ConnectTimeout=3",f"{U}@{H}",f"tmux send-keys -t '{s}' '{safe}' Enter"],capture_output=True,timeout=8)
        except:pass

class V(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.srv=Srv()
        self.rec=False;self.ad=[];self.ast=None;self.txt=""
        self.sr=16000;self._t0=0

        # Fenster
        self.title("");self.configure(fg_color=BG);self.overrideredirect(True)
        self.attributes("-topmost",True,"-alpha",OP)
        sw=self.winfo_screenwidth()
        self.geometry("220x58+"+str(sw//2-110)+"+48")
        self.bind("<Button-1>",self._ds);self.bind("<B1-Motion>",self._dm)
        self._dx=self._dy=0

        # Haupt-Frame (weiß, abgerundet, shadow)
        self.main=ctk.CTkFrame(self,fg_color=W,corner_radius=R)
        self.main.pack(fill="both",expand=True)

        self._idle()
        threading.Thread(target=self._hc,daemon=True).start()

    def _ds(self,e):self._dx,self._dy=e.x,e.y
    def _dm(self,e):
        x=self.winfo_x()+e.x-self._dx;y=self.winfo_y()+e.y-self._dy
        self.geometry(f"220x58+{x}+{y}")

    def _clear(self):
        for w in self.main.winfo_children():w.destroy()

    def _row(self,p=None):
        if p is None:p=self.main
        r=ctk.CTkFrame(p,fg_color="transparent")
        r.pack(fill="both",expand=True,padx=12,pady=0)
        return r

    # ── IDLE ──
    def _idle(self):
        self._clear()
        self.geometry("220x58")
        r=self._row()

        # Mic-Button (rund, lila)
        b=ctk.CTkButton(r,text="🎤",width=42,height=42,corner_radius=21,
            fg_color=PU,hover_color=PUL,text_color=W,font=("Segoe UI",17),
            command=self._toggle,border_width=0)
        b.pack(side="left",padx=(4,10))

        # Label
        ctk.CTkLabel(r,text="Voice",font=("Segoe UI",15,"bold"),text_color=DARK).pack(side="left",padx=(0,6))
        # Status-Dot
        ctk.CTkLabel(r,text="●",font=("Segoe UI",9),text_color=GN).pack(side="right",padx=8)

    # ── RECORDING ──
    def _rec_ui(self):
        self._clear();self.geometry("280x58")
        r=self._row()

        # Stop-Button (rund, rot)
        b=ctk.CTkButton(r,text="⏹",width=42,height=42,corner_radius=21,
            fg_color=RD,hover_color="#e54040",text_color=W,font=("Segoe UI",17),
            command=self._toggle,border_width=0)
        b.pack(side="left",padx=(4,10))

        # Timer
        self._rt=ctk.CTkLabel(r,text="0:00",font=("Segoe UI",16,"bold"),text_color=DARK)
        self._rt.pack(side="left",padx=(0,8))

        # VU-Balken
        self._vu=ctk.CTkLabel(r,text="▁▁▁▁▁▁",font=("Segoe UI",10),text_color=PU)
        self._vu.pack(side="left",padx=(0,4))

    # ── LOADING ──
    def _load_ui(self):
        self._clear();self.geometry("180x58")
        r=self._row()
        ctk.CTkLabel(r,text="⏳",font=("Segoe UI",18),text_color=GY).pack(side="left",padx=(8,6))
        ctk.CTkLabel(r,text="Transkribieren...",font=("Segoe UI",13),text_color=GY).pack(side="left")

    # ── RESULT ──
    def _res_ui(self):
        self._clear();self.geometry("340x110")
        r=self._row()

        # Obere Zeile: Status + X
        top=ctk.CTkFrame(r,fg_color="transparent");top.pack(fill="x",padx=(0,0),pady=(0,4))
        self._sl=ctk.CTkLabel(top,text="✅",font=("Segoe UI",11),text_color=GN)
        self._sl.pack(side="left")
        ctk.CTkButton(top,text="✕",width=22,height=22,corner_radius=11,
            fg_color="transparent",hover_color=GR,text_color=GY,
            font=("Segoe UI",10),command=self._dismiss).pack(side="right")

        # Text
        self._pv=ctk.CTkLabel(r,text="",font=("Segoe UI",12),text_color=FG,
            anchor="w",justify="left",wraplength=300)
        self._pv.pack(fill="x",padx=0,pady=(0,8))

        # Actions
        acts=ctk.CTkFrame(r,fg_color="transparent");acts.pack(fill="x")

        # Copy-Button
        ctk.CTkButton(acts,text="📋",width=36,height=36,corner_radius=18,
            fg_color=PU,hover_color=PUL,text_color=W,font=("Segoe UI",14),
            command=self._cp,border_width=0).pack(side="left",padx=(0,6))

        # Send-Button
        self._tb=ctk.CTkButton(acts,text="📟",width=36,height=36,corner_radius=18,
            fg_color=BL,hover_color=BLL,text_color=W,font=("Segoe UI",14),
            command=self._send,border_width=0,state="disabled")
        self._tb.pack(side="left",padx=(0,6))

        # Session-Dropdown
        self._tv=ctk.StringVar(value="(keine)")
        om=ctk.CTkOptionMenu(acts,variable=self._tv,values=["(keine)"],
            width=110,height=36,corner_radius=18,
            fg_color=GR,button_color=BL,button_hover_color=BLL,
            dropdown_fg_color=W,dropdown_hover_color=BLL,
            text_color=DARK,font=("Segoe UI",12,"bold"))
        om.pack(side="left",padx=(0,0))

        threading.Thread(target=self._lt,daemon=True).start()

    def _toggle(self):
        if self.rec:self._stop()
        else:self._record()

    def _record(self):
        if not HAS_AUDIO:return
        self.rec=True;self.ad=[];self._rec_ui();self._t0=time.time()

        def cb(indata,frames,t,status):
            if not self.rec:return
            self.ad.append(indata.copy())
            try:
                rms=np.abs(indata).mean()
                l=int(rms*200)
                el=int(time.time()-self._t0)
                m,s=el//60,el%60
                vu="█"*min(l//4,6)+"▁"*max(6-min(l//4,6),0)
                self.after(0,lambda v=vu,mm=m,ss=s:(
                    self._vu.configure(text=v)if hasattr(self,'_vu')else None,
                    self._rt.configure(text=f"{mm}:{ss:02d}")if hasattr(self,'_rt')else None,
                ))
            except:pass

        try:
            self.ast=sd.InputStream(samplerate=self.sr,channels=1,dtype="float32",callback=cb)
            self.ast.start()
        except:
            self.rec=False
            self._idle()

    def _stop(self):
        self.rec=False
        if self.ast:
            try:self.ast.stop();self.ast.close()
            except:pass
            self.ast=None

        # Mindestens 0.5s aufnahme
        if len(self.ad)==0:
            self._idle();return

        self._load_ui()
        threading.Thread(target=self._tx,daemon=True).start()

    def _tx(self):
        try:
            arr=np.concatenate(self.ad,axis=0)
            buf=io.BytesIO()
            sf.write(buf,arr,self.sr,format="WAV")
            wav=buf.getvalue()
            r=self.srv.x(wav)
            self.after(0,lambda res=r:self._rx(res))
        except Exception as ex:
            print(f"_tx error: {ex}")
            self.after(0,self._idle)

    def _rx(self,r):
        if "error" in r:
            print(f"rx error: {r}")
            self._idle();return
        self.txt=r.get("text","")
        if not self.txt.strip():self._idle();return
        lang=r.get("language","?");conf=r.get("language_probability",0)*100;dur=r.get("duration_s",0)
        self._res_ui()
        self._sl.configure(text=f"✅  {lang.upper()}  {conf:.0f}%  ·  {dur:.1f}s")
        self._pv.configure(text=self.txt[:100]+("…"if len(self.txt)>100 else""))
        self.clipboard_clear();self.clipboard_append(self.txt)

    def _cp(self):
        self.clipboard_clear();self.clipboard_append(self.txt)
        self._sl.configure(text="📋 Kopiert!")
        self.after(1500,lambda:self._sl.configure(text=""))

    def _lt(self):
        ss=self.srv.tmux()
        if ss:
            self.after(0,lambda s=ss:(self._tv.set(s[0]),self._tb.configure(state="normal"),
                       self._tb.master.winfo_children()[2].configure(values=s)))

    def _send(self):
        s=self._tv.get()
        if s and self.txt and not s.startswith("("):
            self.srv.st(s,self.txt)
            self._sl.configure(text="📨 Gesendet!")
            self.after(1500,self._dismiss)

    def _dismiss(self):
        self.txt=""
        self._idle()

    def _hc(self):
        # einmal beim start
        r=self.srv.check()
        ok="error" not in r

if __name__=="__main__":
    ctk.set_appearance_mode("light");ctk.set_default_color_theme("dark-blue")
    app=V()
    try:app.mainloop()
    except:pass